...
function interpolate(e, t) {
	return e.reduce((e, r, s) => e + r + (null == t[s] ? "" : t[s]), "");
}
function uuid() {
...
function waitForLoad(e, t = 2e3) {
	return new Promise((r) => {
		const s = () => {
			e.removeEventListener("error", s), e.removeEventListener("load", s), r();
		};
...
function getCspNonce() {
	const e = getMetaElement("csp-nonce");
	if (e) {
...
function debounce(e, t) {
	let r = null;
	return (...s) => {
		clearTimeout(r), (r = setTimeout(() => e.apply(this, s), t));
	};
}
const submitter = {
	"aria-disabled": {
		beforeSubmit: (e) => {
			e.setAttribute("aria-disabled", "true"),
				e.addEventListener("click", cancelEvent);
		},
		afterSubmit: (e) => {
			e.removeAttribute("aria-disabled"),
				e.removeEventListener("click", cancelEvent);
		},
	},
	disabled: {
...
class Config {
	...
	set submitter(e) {
		this.#e = submitter[e] || e;
	}
}
const forms = new Config({ mode: "on", submitter: "disabled" }),
	config = { drive: drive, forms: forms };
...
function locationIsVisitable(e, t) {
	return (
		isPrefixedBy(e, t) &&
		!config.drive.unvisitableExtensions.has(getExtension(e))
	);
}
...
class FetchRequest {
	...
	constructor(
		...
		const [o, a] = buildResourceAndBody(expandURL(r), t, s, n);
		(this.delegate = e),
			(this.url = o),
			(this.target = i),
			(this.fetchOptions = {
	...
	set method(e) {
		...
		this.url.search = "";
		const [s, i] = buildResourceAndBody(this.url, r, t, this.enctype);
		(this.url = s),
	...
	async #r(e) {
		...
		return (this.url = r.detail.url), r.defaultPrevented && (await t), r;
	}
	#s(e) {
...
class PrefetchCache {
	...
	setLater(e, t, r) {
		this.clear(),
			(this.#i = setTimeout(() => {
				t.perform(), this.set(e, t, r), (this.#i = null);
			}, 100));
	}
	set(e, t, r) {
		this.#n = {
...
class FormSubmission {
	...
	constructor(e, t, r, s = !1) {
		const i = getMethod(t, r),
			n = getAction(getFormAction(t, r), i),
			o = buildFormData(t, r),
			a = getEnctype(t, r);
		(this.delegate = e),
			(this.formElement = t),
	...
	async start() {
		...
		if ("string" == typeof r) {
			const e =
				"function" == typeof config.forms.confirm
					? config.forms.confirm
					: FormSubmission.confirmMethod;
			if (!(await e(r, this.formElement, this.submitter))) return;
		}
	...
	requestStarted(e) {
		(this.state = FormSubmissionState.waiting),
			this.submitter && config.forms.submitter.beforeSubmit(this.submitter),
			this.setSubmitsWith(),
	...
	requestFinished(e) {
		(this.state = FormSubmissionState.stopped),
			this.submitter && config.forms.submitter.afterSubmit(this.submitter),
...
function buildFormData(e, t) {
	...
	return s && r.append(s, i || ""), r;
}
function getCookieValue(e) {
	if (null != e) {
		const t = (document.cookie ? document.cookie.split("; ") : []).find((t) =>
			t.startsWith(e),
		);
		if (t) {
...
function submissionDoesNotDismissDialog(e, t) {
	return (
		"dialog" != (t?.getAttribute("formmethod") || e.getAttribute("method"))
	);
}
function submissionDoesNotTargetIFrame(e, t) {
	return doesNotTargetIFrame(
		t?.getAttribute("formtarget") || e.getAttribute("target"),
	);
}
class View {
	#o = (e) => {};
	...
	async render(e) {
		...
		if (r)
			try {
				...
				this.delegate.allowsImmediateRender(i, s) || (await r),
...
class LinkClickObserver {
	...
	clickBubbled = (e) => {
		if (e instanceof MouseEvent && this.clickEventIsSignificant(e)) {
			const t = findLinkFromClickTarget(
				(e.composedPath && e.composedPath()[0]) || e.target,
			);
			if (t && doesNotTargetIFrame(t.target)) {
				const r = getLocationForLink(t);
				this.delegate.willFollowLinkToLocation(t, r, e) &&
					(e.preventDefault(), this.delegate.followedLinkToLocation(t, r));
			}
		}
	};
	clickEventIsSignificant(e) {
...
class FormLinkClickObserver {
	...
	followedLinkToLocation(e, t) {
		...
		const s = Object.assign(t, { search: "" });
		...
		const o = getVisitAction(e);
		o && r.setAttribute("data-turbo-action", o);
	...
	enteringBardo(e) {
		this.#c ||
			(e.contains(this.currentSnapshot.activeElement) &&
				(this.#c = this.currentSnapshot.activeElement));
	}
	leavingBardo(e) {
		e.contains(this.#c) &&
			this.#c instanceof HTMLElement &&
			(this.#c.focus(), (this.#c = null));
	}
...
class FrameRenderer extends Renderer {
	static renderElement(e, t) {
		...
		const s = t,
			i = s.ownerDocument?.createRange();
		i && (i.selectNodeContents(s), e.appendChild(i.extractContents()));
	}
	constructor(e, t, r, s, i, n = !0) {
		super(t, r, s, i, n), (this.delegate = e);
	}
	get shouldRender() {
		return !0;
	}
	...
	const r = (function () {
			function e(e, t, r, i) {
				...
				if (i.idMap.has(t)) {
					...
					return (
						e.insertBefore(n, r), s(n, t, i), i.callbacks.afterNodeAdded(n), n
					);
				}
				{
					const s = document.importNode(t, !0);
					return e.insertBefore(s, r), i.callbacks.afterNodeAdded(s), s;
				}
			}
			const t = (function () {
				function e(e, t, r) {
				...
				return function (r, s, i, n) {
					...
					for (; l && l != n; ) {
						if (t(l, s)) {
							if (e(r, l, s)) return l;
							null === o && (r.idMap.has(l) || (o = l));
						}
						if (
							(null === o &&
								a &&
								t(l, a) &&
								(c++, (a = a.nextSibling), c >= 2 && (o = void 0)),
			...
			function n(e, t, r, s) {
				const i =
					s.target.querySelector(`#${t}`) || s.pantry.querySelector(`#${t}`);
				return (
					(function (e, t) {
						const r = e.id;
						for (; (e = e.parentNode); ) {
							let s = t.idMap.get(e);
							s && (s.delete(r), s.size || t.idMap.delete(e));
						}
					})(i, s),
					o(e, i, r),
					i
			...
			return function (o, a, c, l = null, h = null) {
				a instanceof HTMLTemplateElement &&
					c instanceof HTMLTemplateElement &&
					((a = a.content), (c = c.content)),
					(l ||= a.firstChild);
				for (const r of c.childNodes) {
					if (l && l != h) {
						const e = t(o, r, l, h);
						if (e) {
							e !== l && i(o, l, e), s(e, r, o), (l = e.nextSibling);
							continue;
						}
					}
					if (r instanceof Element && o.persistentIds.has(r.id)) {
						const e = n(a, r.id, l, o);
						s(e, r, o), (l = e.nextSibling);
						continue;
					}
					const c = e(a, r, l, o);
					c && (l = c.nextSibling);
		...
		s = (function () {
			function e(e, r, s, i) {
				const n = r[s];
				if (n !== e[s]) {
					const o = t(s, e, "update", i);
					o || (e[s] = r[s]),
						n
							? o || e.setAttribute(s, "")
							: t(s, e, "remove", i) || e.removeAttribute(s);
				}
			}
			function t(e, t, r, s) {
				return (
					...
					) || !1 === s.callbacks.beforeAttributeUpdated(e, t, r)
				);
			}
			function s(e, t) {
				return (
								...
								(n instanceof HTMLHeadElement && "morph" !== a.head.style
									...
									: (!(function (r, i, n) {
											...
											if (1 === o) {
												...
												for (const e of l)
													t(e.name, o, "update", n) ||
														(o.getAttribute(e.name) !== e.value &&
															o.setAttribute(e.name, e.value));
												for (let e = c.length - 1; 0 <= e; e--) {
													const r = c[e];
													if (r && !a.hasAttribute(r.name)) {
														if (t(r.name, o, "remove", n)) continue;
												...
												s(o, n) ||
													(function (r, s, i) {
														if (
															...
															let e = s.value,
																n = r.value;
															if (t("value", r, "update", i)) return;
															e !== n && (r.value = e),
																r.firstChild &&
																	r.firstChild.nodeValue !== e &&
																	(r.firstChild.nodeValue = e);
														}
													})(o, a, n);
											}
											(8 !== o && 3 !== o) ||
												(r.nodeValue !== i.nodeValue &&
													(r.nodeValue = i.nodeValue));
										})(n, o, a),
										s(n, a) || r(a, n, o))),
	...
	function i(e, t, r) {
		...
		for (const t of e.children) {
			let e = a.has(t.outerHTML),
				s = r.head.shouldReAppend(t),
				c = r.head.shouldPreserve(t);
			e || c
				? s
					? i.push(t)
				...
				: "append" === r.head.style
					? s && (i.push(t), o.push(t))
					: !1 !== r.head.shouldRemove(t) && i.push(t);
		}
		o.push(...a.values());
		let c = [];
		for (const t of o) {
			...
			if (!1 !== r.callbacks.beforeNodeAdded(i)) {
				...
				e.appendChild(i), r.callbacks.afterNodeAdded(i), s.push(i);
			}
		}
		for (const t of i)
			!1 !== r.callbacks.beforeNodeRemoved(t) &&
				(e.removeChild(t), r.callbacks.afterNodeRemoved(t));
		return r.head.afterHeadMorphed(e, { added: s, kept: n, removed: i }), c;
	}
	const n = (function () {
			function e() {
				const e = document.createElement("div");
				return (
					(e.hidden = !0), document.body.insertAdjacentElement("afterend", e), e
				);
			}
			function r(e) {
				...
				const { persistentIds: a, idMap: c } = (function (e, t) {
						const i = r(e),
							...
							o = (function (e, t) {
								let r = new Set(),
									s = new Map();
								for (const { id: t, tagName: i } of e)
									s.has(t) ? r.add(t) : s.set(t, i);
								let i = new Set();
						...
						return s(a, o, c, n), { persistentIds: o, idMap: a };
					})(i, n),
					l = (function (e) {
		...
		{ normalizeElement: o, normalizeParent: a } = (function () {
			...
			return {
				...
				normalizeParent: function t(r) {
					if (null == r) return document.createElement("div");
					if ("string" == typeof r)
						return t(
							(function (t) {
								let r = new DOMParser(),
									s = t.replace(/<svg(\s[^>]*>|>)([\s\S]*?)<\/svg>/gim, "");
								if (
									s.match(/<\/html>/) ||
									s.match(/<\/head>/) ||
									s.match(/<\/body>/)
									...
									if (s.match(/<\/html>/)) return e.add(i), i;
									{
										let t = i.firstChild;
										return t && e.add(t), t;
									}
							...
							return (function (e) {
								return {
									...
									querySelectorAll: (t) => {
										...
										return e.matches(t) ? [e, ...r] : r;
									},
									insertBefore: (t, r) => e.parentNode.insertBefore(t, r),
									moveBefore: (t, r) => e.parentNode.moveBefore(t, r),
									get __idiomorphRoot() {
										return e;
									},
								};
			...
			const c = a(t),
				...
				h = (function (e, t) {
					...
					s &&
						...
						((r = e.target.querySelector(`#${s}`)), r?.focus());
					...
					(function (e, t, r, s) {
						...
						"innerHTML" === t.morphStyle
							? (r(t, e, c), Array.from(e.childNodes))
							: (function (e, t, s) {
									const i = a(t);
									...
									const o = n.indexOf(t),
										c = n.length - (o + 1);
									return (
										r(e, i, s, t, t.nextSibling),
										(n = Array.from(i.childNodes)),
										n.slice(o, n.length - c)
									);
								})(t, e, c),
					),
				);
...
function morphElements(e, t, { callbacks: r, ...s } = {}) {
	Idiomorph.morph(e, t, { ...s, callbacks: new DefaultIdiomorphCallbacks(r) });
}
function morphChildren(e, t) {
	morphElements(e, t.childNodes, { morphStyle: "innerHTML" });
}
class DefaultIdiomorphCallbacks {
	...
	beforeAttributeUpdated = (e, t, r) =>
		!dispatch("turbo:before-morph-attribute", {
			cancelable: !0,
			target: t,
...
class ProgressBar {
	...
	createStylesheetElement() {
		...
		(e.type = "text/css"), (e.textContent = ProgressBar.defaultCSS);
		const t = getCspNonce();
...
class PageSnapshot extends Snapshot {
	...
	clone() {
		...
		for (const [e, s] of t.entries()) {
			...
			for (const e of s.selectedOptions) t.options[e.index].selected = !0;
		}
...
class Visit {
	...
	changeHistory() {
		if (!this.historyChanged && this.updateHistory) {
			const e = getHistoryMethodForAction(
...
class FrameRedirector {
	...
	#p(e, t) {
		const r = getAction$1(e, t),
			s = this.element.ownerDocument.querySelector('meta[name="turbo-root"]'),
			i = expandURL(s?.content ?? "/");
		return this.#u(e, t) && locationIsVisitable(r, i);
	}
	#u(e, t) {
		if (
...
class LinkPrefetchObserver {
	...
	start() {
		this.started ||
			("loading" === this.eventTarget.readyState
				? this.eventTarget.addEventListener("DOMContentLoaded", this.#g, {
						once: !0,
					})
				: this.#g());
	}
	stop() {
		this.started &&
			(this.eventTarget.removeEventListener("mouseenter", this.#b, {
				capture: !0,
				passive: !0,
			}),
			this.eventTarget.removeEventListener("mouseleave", this.#v, {
				capture: !0,
				passive: !0,
			}),
	...
	#g = () => {
		this.eventTarget.addEventListener("mouseenter", this.#b, {
			capture: !0,
			passive: !0,
		}),
			this.eventTarget.addEventListener("mouseleave", this.#v, {
				capture: !0,
				passive: !0,
			}),
	...
	#b = (e) => {
		if ("false" === getMetaContent("turbo-prefetch")) return;
		const t = e.target;
		if (
			t.matches &&
			...
			if (this.delegate.canPrefetchRequestToLocation(e, r)) {
				...
				prefetchCache.setLater(r.toString(), s, this.#y);
			}
		}
	};
	#v = (e) => {
		e.target === this.#f && this.#w();
	};
	#w = () => {
		prefetchCache.clear(), (this.#f = null);
	};
	#S = (e) => {
		if ("FORM" !== e.target.tagName && "GET" === e.detail.fetchOptions.method) {
			const t = prefetchCache.get(e.detail.url.toString());
			t && (e.detail.fetchRequest = t), prefetchCache.clear();
		}
	};
	prepareRequest(e) {
		const t = e.target;
		...
		const r = t.closest("turbo-frame"),
			s =
				...
				r?.id;
		s && "_top" !== s && (e.headers["Turbo-Frame"] = s);
	}
	...
	requestFailedWithResponse(e, t) {}
	get #y() {
		return Number(getMetaContent("turbo-prefetch-cache-time")) || 1e4;
	}
	#E(e) {
		return (
			!!e.getAttribute("href") &&
...
const unfetchableLink = (e) =>
	...
	eventPrevented = (e) =>
		dispatch("turbo:before-prefetch", { target: e, cancelable: !0 })
			.defaultPrevented;
class Navigator {
	constructor(e) {
		this.delegate = e;
	...
	locationWithActionIsSamePage(e, t) {
		const r = getAnchor(e),
			s = getAnchor(this.view.lastRenderedLocation),
			i = "restore" === t && void 0 === r;
		return (
			"replace" !== t &&
	...
	#R(e, t) {
		...
		return getVisitAction(r, s) || this.#L(t);
	}
	#L(e) {
...
const PageStage = { initial: 0, loading: 1, interactive: 2, complete: 3 };
class PageObserver {
...
async function withAutofocusFromFragment(e, t) {
	const r = `turbo-stream-autofocus-${uuid()}`,
		s = firstAutofocusableElementInStreams(e.querySelectorAll("turbo-stream"));
	let i = null;
	s && ((i = s.id ? s.id : r), (s.id = i)), t(), await nextRepaint();
	if (
...
class StreamObserver {
	...
	receiveMessageEvent = (e) => {
		this.#A && "string" == typeof e.data && this.receiveMessageHTML(e.data);
	};
...
class PageRenderer extends Renderer {
	...
	isCurrentElementInElementList(e, t) {
		for (const [r, s] of t.entries()) {
			if ("TITLE" == e.tagName) {
				if ("TITLE" != s.tagName) continue;
				if (e.innerHTML == s.innerHTML) return t.splice(r, 1), !0;
			}
			if (s.isEqualNode(e)) return t.splice(r, 1), !0;
		}
		return !1;
	}
...
class PageView extends View {
	...
	renderPage(e, t = !1, r = !0, s) {
		const i = new (
			this.isPageRefresh(s) && this.snapshot.shouldMorphPage
				? MorphingPageRenderer
				: PageRenderer
		)(this.snapshot, e, t, r);
		return (
			i.shouldRender ? s?.changeHistory() : (this.forceReloaded = !0),
			this.render(i)
		);
	}
	renderError(e, t) {
		t?.changeHistory();
		const r = new ErrorRenderer(this.snapshot, e, !1);
...
class Preloader {
	...
	start() {
		"loading" === document.readyState
			? document.addEventListener("DOMContentLoaded", this.#P)
			: this.preloadOnLoadLinksForView(document.body);
	}
	stop() {
		document.removeEventListener("DOMContentLoaded", this.#P);
	}
	...
	async requestSucceededWithResponse(e, t) {
		try {
			const r = await t.responseHTML,
				s = PageSnapshot.fromHTMLString(r);
...
class Session {
	...
	refresh(e, t) {
		const r = t && this.recentRequests.has(t),
			s = e === document.baseURI;
		r ||
			this.navigator.currentVisit ||
			!s ||
			this.visit(e, { action: "replace", shouldCacheSnapshot: !1 });
	}
	connectStreamSource(e) {
	...
	set progressBarDelay(e) {
		config.drive.progressBarDelay = e;
	}
	get progressBarDelay() {
		return config.drive.progressBarDelay;
	}
	set drive(e) {
		config.drive.enabled = e;
	}
	get drive() {
	...
	shouldPreloadLink(e) {
		const t = e.hasAttribute("data-turbo-method"),
			...
			s = e.getAttribute("data-turbo-frame"),
			i =
				"_top" == s
					? null
					: document.getElementById(s) ||
						findClosestRecursively(e, "turbo-frame:not([disabled])");
		if (t || r || i instanceof FrameElement) return !1;
		{
			const t = new URL(e.href);
			return (
	...
	allowsImmediateRender({ element: e }, t) {
		...
		return (
			this.view.renderer && i && (this.view.renderer.renderElement = i), !s
		);
	}
	...
	applicationAllowsFollowingLinkToLocation(e, t, r) {
		return !this.notifyApplicationAfterClickingLinkToLocation(e, t, r)
			.defaultPrevented;
	}
	...
	submissionIsNavigatable(e, t) {
		if ("off" == config.forms.mode) return !1;
		{
			const r = !t || this.elementIsNavigatable(t);
			return "optin" == config.forms.mode
...
const deprecatedLocationPropertyDescriptors = {
	...
	{ cache: cache, navigator: navigator$1 } = session;
function start() {
	session.start();
}
...
class FrameController {
	...
	disabledChanged() {
		this.loadingStyle == FrameLoadingStyle.eager && this.#x();
	}
	sourceURLChanged() {
		this.#O("src") ||
			(this.element.isConnected && (this.complete = !1),
			(this.loadingStyle == FrameLoadingStyle.eager || this.#q) && this.#x());
	}
	sourceURLReloaded() {
	...
	loadingStyleChanged() {
		this.loadingStyle == FrameLoadingStyle.lazy
			? this.appearanceObserver.start()
			: (this.appearanceObserver.stop(), this.#x());
	}
	async #x() {
		this.enabled &&
			this.isActive &&
	...
	async loadResponse(e) {
		...
		try {
			...
			if (t) {
				...
				PageSnapshot.fromDocument(r).isVisitable
					? await this.#V(e, r)
					: await this.#D(e);
			}
		} finally {
			(this.#B = !1), (this.fetchResponseLoaded = () => Promise.resolve());
		}
	}
	elementAppearedInViewport(e) {
		this.proposeVisitIfNavigatedWithAction(e, getVisitAction(e)), this.#x();
	}
	...
	willSubmitForm(e, t) {
		return e.closest("turbo-frame") == this.element && this.#W(e, t);
	}
	formSubmitted(e, t) {
	...
	async requestSucceededWithResponse(e, t) {
		await this.loadResponse(t), this.#k();
	}
	async requestFailedWithResponse(e, t) {
		await this.loadResponse(t), this.#k();
	}
	requestErrored(e, t) {
		console.error(t), this.#k();
	}
	requestFinished(e) {
		clearBusyState(this.element);
	}
	formSubmissionStarted({ formElement: e }) {
		markAsBusy(e, this.#m(e));
	}
	...
	async #V(e, t) {
		const r = await this.extractForeignFrameElement(t.body),
			s = this.#B ? MorphingFrameRenderer : FrameRenderer;
		if (r) {
			const t = new Snapshot(r),
				i = new s(this, this.view.snapshot, t, !1, !1);
			this.view.renderPromise && (await this.view.renderPromise),
				this.changeHistory(),
		...
		if (((this.action = t), this.action)) {
			...
			e.delegate.fetchResponseLoaded = async (s) => {
				if (e.src) {
					const { statusCode: i, redirected: n } = s,
						o = {
							response: {
								statusCode: i,
								redirected: n,
								responseHTML: await s.responseHTML,
							},
					...
					this.action && (o.action = this.action), session.visit(e.src, o);
				}
			};
		}
	}
	...
	#$(e) {
		...
		return !dispatch("turbo:frame-missing", {
			...
			detail: {
				response: t,
				visit: async (e, t) => {
					e instanceof Response ? this.#_(e) : session.visit(e, t);
				},
			},
			cancelable: !0,
		}).defaultPrevented;
	...
	async #_(e) {
		const t = new FetchResponse(e),
			...
			{ location: s, redirected: i, statusCode: n } = t;
		return session.visit(s, {
	...
	#W(e, t) {
		...
		return (
			!!session.elementIsNavigatable(e) &&
			!(t && !session.elementIsNavigatable(t))
		);
	}
	get id() {
	...
	get isLoading() {
		return void 0 !== this.formSubmission || void 0 !== this.#k();
	}
	get complete() {
	...
	#J(e, t) {
		this.#H.add(e), t(), this.#H.delete(e);
	}
	#j(e, t) {
...
const StreamActions = {
	...
	update() {
		...
		this.targetElements.forEach((t) => {
			"morph" === e
				? morphChildren(t, this.templateContent)
	...
	get targetElements() {
		return this.target
			...
			: this.targets
				...
				: void this.#Y("target or targets attribute is missing");
	}
	get templateContent() {
...
(FrameElement.delegateConstructor = FrameController),
	...
	(window.Turbo = { ...Turbo, StreamActions: StreamActions }),
	start();
export {
	FetchEnctype,
	FetchMethod,
